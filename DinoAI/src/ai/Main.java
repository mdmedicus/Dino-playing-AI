...
public class Main {
	...
	public int SonucuAl() {
		...
		for(int i = 0; i< str.layers.size()-1; i++) {
			...
			} else if(str.f_type.get(i) == 1){
				core.actN_sigmoid(i+1);
			} else if(str.f_type.get(i) == 2) {
				core.actN_softmax(i+1);
			}
...
//		core.bpCNN();
	...
	public void Rl(float rt, float Q, int S) {
		core.RLgradient(rt, Q, S);
	}
