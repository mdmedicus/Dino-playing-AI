...
import java.util.Arrays;
	...
	public class Core {
			...
			bestOne = new ArrayList<>();
				...
				for(int y = 0; y < str.newSize; y++) {
					for(int x = 0; x < str.newSize; x++) {
						...
						str.pooling[i][y*str.newSize + x] = sonuc/(str.filterSize*str.filterSize*str.filterChannel);
					}
				}
		...
		public void actN_softmax(int i) {
			float toplam = 0;
			for(int k = 0; k < str.layers.get(i).length; k++) {
				float z = 0;
				
				for(int j = 0; j < str.layers.get(i-1).length; j++) {
					z = z + (float)str.layers.get(i-1)[j]*str.weights.get(i-1)[k*str.layers.get(i-1).length+j];
				}
				
				toplam += (float)Math.exp(z);
				
				str.layers.get(i)[k] = (float) Math.exp(z);
				
			}
			
			for(int k = 0; k < str.layers.get(i).length; k++) {
				
				str.layers.get(i)[k] = (float)str.layers.get(i)[k]/toplam;
...
//			System.out.println("0:"+(starttime00 - System.nanoTime()));
		...
		//	long starttime0 = System.nanoTime();
			for(int m = 0; m < str.layers.get(str.layers.size()-1).length; m++) {
				...
				} else if(str.f_type.get(str.f_type.size()-1) == 1 || str.f_type.get(str.f_type.size()-1) == 2 ){
...
//			long startime = System.nanoTime();
				...
				for(int m = 0; m < str.layers.get(layerNumber).length; m++) {
					...
					if(str.f_type.get(layerNumber-1) == 0) {
						...
						if(str.layers.get(layerNumber)[m] > 0) {
							...
							Cz.get(layerNumber-1)[m] = Ca;
							
						} else {
							Cz.get(layerNumber-1)[m] = 0;
						}
					} else {
						for(int k = 0; k < str.layers.get(layerNumber+1).length; k ++) {
							Ca = Ca + (float)Cz.get(layerNumber)[k]*str.weights.get(layerNumber)[k*str.layers.get(layerNumber).length+m];
	...
	/*		for(int i = 1; i < layers.size(); i++) {
		...
		public void RLgradient(float rt, float Q, int S) {
			float[] outputz = new float[str.layers.get(str.layers.size()-1).length];	
			outputz[S] = Q*(1-Q);
			
			for(int m = 0; m < str.layers.get(str.layers.size()-1).length; m++) {
				for(int n = 0; n < str.layers.get(str.layers.size()-2).length; n++) {
					
					str.weights.get(str.weights.size()-1)[m*str.layers.get(str.layers.size()-2).length +n] += (float)str.lr*outputz[m]*rt;
					
				}
			}
			
			
			ArrayList<float[]> dO_z = new ArrayList<>();
			dO_z.add(outputz);
			
			
			for(int i = 0; i < str.weights.size()-1; i++)	{
				float[] qz = new float[str.layers.get(str.layers.size()-2-i).length];
				
				for(int m = 0; m < str.layers.get(str.layers.size()-2-i).length; m++) {
					
					
					for(int k = 0; k < str.layers.get(str.layers.size()-2-i+1).length; k++) {
						
						
						if(str.f_type.get(str.f_type.size()-1) == 0) {
							
							if(str.layers.get(str.layers.size()-1)[m] > 0) {
								qz[m] += (float)dO_z.get(i)[k]*str.weights.get(str.weights.size()-1-i)[k*str.layers.get(str.layers.size()-2-i).length+m];
										
							} else{
								qz[m] += 0;
							}
						} else if(str.f_type.get(str.f_type.size()-1) == 1 || str.f_type.get(str.f_type.size()-1) == 2 ){
								qz[m] += (float)dO_z.get(i)[k]*
										str.weights.get(str.weights.size()-1-i)[k*str.layers.get(str.layers.size()-2-i).length+m]*
										str.layers.get(str.layers.size()-2-i)[m]*(1-str.layers.get(str.layers.size()-2-i)[m]);
					...
					for(int n = 0; n < str.layers.get(str.layers.size()-3-i).length; n++) {
						
						
						str.weights.get(str.weights.size()-2-i)[m*str.layers.get(str.layers.size()-3-i).length+n] +=(float)
								str.lr*qz[m]*str.layers.get(str.layers.size()-3-i)[n];
